...
- 移除 `package.json` 中的 `packageManager` 字段
- 删除 `.npmrc` 和 `pnpm-lock.yaml` 文件
- 简化 `vercel.json` 配置，移除自定义安装命令
- Vercel 将使用默认的 npm 安装依赖
- 本地仍然可以使用 pnpm 开发

**为什么切换到 npm**:
- pnpm 在 Vercel 环境中存在兼容性问题
- npm 是 Vercel 默认且最稳定的包管理器
- 不需要复杂的配置和锁定文件

### 之前的尝试（已废弃）

**尝试 1**: 添加 npm 官方源和清除缓存
**尝试 2**: 添加 .nvmrc 和环境变量
**尝试 3**: 使用 --no-frozen-lockfile 参数

这些方案都未能解决问题，最终选择切换到 npm。
